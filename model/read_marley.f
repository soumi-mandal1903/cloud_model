      subroutine read_marley( input_file, 
...
c   convert from m/s^2 to cm/s^2
c      grav=grav*1e2
      print*, teff, grav, nzp1
...
c   (convert pressure in bars to dyne/cm^2)
      ...
      if( nzp1-1 .gt. MAXNZ )then 
        print*, 'read_marley(): nz > MAXNZ'
        stop 1
      endif
 
      do iz = 1, nzp1
        read(LUNIO,*) idummy, dum1, dum2, dum3
C        read(LUNIO,*) dum1, dum2
        print*, dum1, dum2
        t_top(iz) = dum2
        if( dum1 .eq. 0. )then
          p_top(iz) = dum3*1e6
